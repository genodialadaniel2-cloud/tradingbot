...
- **Polling frequency** — `signal_bot.py` polls hourly (`POLL_SECONDS`) so new 1H CRT candles are caught promptly; the 4H RSI zone and 4H CRT check just get re-evaluated redundantly within their own window, which is harmless since alerts are deduped per closed candle.
